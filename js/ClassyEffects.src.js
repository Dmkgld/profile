...
function ClassyEffects(params) {
    ...
    this.effectObj = function (width, height, canvas) {
        ...
        this.update = function () {
            ...
            this.ctx.beginPath();
            let a = this.onStageList.first;
            for (let d = 0; d < this.numOnStage; d++) {
            ...
            if (this.colorTransform === true) {
                let g = this.ctx.getImageData(0, 0, this.displayWidth, this.displayHeight);
                let e = g.data;
        ...
        this.splash = function (h, c) {
            let g = Math.ceil(this.minSplashDrops + Math.random() * (this.maxSplashDrops - this.minSplashDrops));
            for (let e = 0; e < g - 1; e++) {
                let a = 0.75 + 0.75 * Math.random();
                let f = a * (this.splashMinVelX + Math.random() * (this.splashMaxVelX - this.splashMinVelX));
                let d = -a * (this.splashMinVelY + Math.random() * (this.splashMaxVelY - this.splashMinVelY));
                let b = this.droplet(h, this.displayHeight, c, this.splashThickness, f, d, this.splashAlpha, false);
                b.breakawayTime = 0;
            }
        };
    };
    ...
    this.__constructor = function (params) {
        let _c = params.container;
        this.effect = new self.effectObj(_c.width(), _c.height(), _c[0]);
        switch (params.effect) {
            case 'rain':
                this.effect.colorTransform = true;
                this.effect.dropThickness = 1;
                this.effect.dropAlpha = 0.5;
                this.effect.splashAlpha = 0.5;
                this.effect.defaultInitialVelocity = new self.pointObj(0, 11);
                this.effect.splashMaxVelX = .3;
                this.effect.splashMinVelX = -.3;
                this.effect.splashMinVelY = 0.2;
                this.effect.splashMaxVelY = 1.0;
                this.effect.minSplashDrops = 1;
                this.effect.maxSplashDrops = 15;
                this.effect.removeDropsOutsideXRange = false;
                this.effect.breakawayTimeVariance = 0.01;
                this.effect.wind = new self.pointObj(1, 1);
                this.effect.waitCount = 10;
